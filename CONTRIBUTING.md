...
# Working on the Rust HTTP client

The HTTP client lives in `rust/gitea-client/`. It is a standalone Rust crate that can be developed, built, and tested independently of Jenkins — no plugin reload or Jenkins runtime is required to exercise it.

## Layout

* `src/auth.rs` — `Auth` enum (`None` / `Token` / `Basic`) and header application. Note Gitea tokens use the `Authorization: token <T>` scheme, not `Bearer`.
* `src/error.rs` — `GiteaError` enum, mapped to Java exceptions (`GiteaHttpStatusException`, `IOException`) at the JNI boundary.
* `src/runtime.rs` — lazy Tokio `Runtime` and pooled `reqwest::Client` shared across all JNI calls.
* `src/client.rs` — `GiteaClient` with 33 Gitea API endpoints. Each method is async and returns `Result<String, GiteaError>` where the `String` is raw JSON.
* `src/lib.rs` — crate root; re-exports the public API and (once stage 2 lands) hosts the `#[no_mangle] extern "system" fn` JNI exports.
* `tests/integration.rs` — `wiremock`-based tests for every endpoint, including 404 handling, pagination, and auth header assertions.

## Testing

```bash
cd rust/gitea-client
cargo test                 # all integration tests use wiremock — no live Gitea needed
cargo fmt -- --check
cargo clippy --all-targets -- -D warnings
```

The integration tests spin up an in-process `wiremock` server, so they are hermetic and parallel-safe.

## Adding a new endpoint

When adding support for a new Gitea API operation, update all layers in lock-step:

1. Add the method to `rust/gitea-client/src/client.rs` — async, returns `Result<String, GiteaError>` containing the raw JSON response.
2. Add a `#[no_mangle] extern "system" fn` export named `Java_org_jenkinsci_plugin_gitea_client_impl_RustGiteaConnection_<methodName>` in the JNI layer.
3. Add a `private static native String native<MethodName>(...)` declaration to `RustGiteaConnection.java`.
4. Add an `@Override public <POJO> <methodName>(...)` to `RustGiteaConnection.java` that invokes the native method and parses the returned JSON via the existing Jackson `ObjectMapper`.
5. Add a `wiremock` test in `rust/gitea-client/tests/integration.rs` that asserts both the request shape (URL, headers, auth) and the parsed result.

## Coding standards

* **Rust:** `cargo fmt` and `cargo clippy --all-targets -- -D warnings` must pass. Public functions carry `///` doc comments. Errors flow through `GiteaError`; do not `unwrap()` on user-controlled input.
* **Java:** the existing checkstyle + spotbugs configuration from the upstream `pom.xml` still applies. The general rules above (4-space indent, no `*` imports in `src/main`, Javadoc on every public API, `@since FIXME` on new types) apply unchanged.
